...
class RAGGenerator:
    ...
    async def stream_rag(self, query, retrieval_results, session_id, history=None, user_name="", web_context="", user_id=None) -> AsyncGenerator[str, None]:
    ...
    async def stream_casual(self, query, session_id, history=None, user_name="", user_id=None) -> AsyncGenerator[str, None]:
        if history is None: history = self.session_mgr.get_history(session_id)
    ...
    async def generate_rag(self, query, retrieval_results, session_id, history=None, user_name="", web_context="", user_id=None) -> dict:
        import asyncio
        if history is None: history = self.session_mgr.get_history(session_id)
        context = format_context_with_labels(retrieval_results) if retrieval_results else ""
        if web_context:
